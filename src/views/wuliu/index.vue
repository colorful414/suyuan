<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="发货时间" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.timefahuo | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="订单号" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.dingdaninum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="发货地点" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.productfahuolocation }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="800px">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="发货时间" prop="timefahuo">
          <el-date-picker v-model="temp.timefahuo" type="datetime" placeholder="Please pick a date" />
        </el-form-item>
        <el-form-item label="订单号" prop="productfahuolocation">
          <el-input v-model="temp.dingdaninum" />
        </el-form-item>
        <el-form-item label="发货地点" prop="productfahuolocation">
          <el-input v-model="temp.productfahuolocation" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        timefahuo: [{ type: 'date', required: true, message: 'timefahuo is required', trigger: 'change' }],
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.temp.timefahuo = new Date(this.temp.timefahuo)
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.timefahuo = +new Date(tempData.timefahuo) // change Thu Nov 30 2017 16:41:05 GMT+0800 (CST) to 1512031311464
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['timefahuo', 'title', 'type', 'importance', 'status']
        const filterVal = ['timefahuo', 'title', 'type', 'importance', 'status']
