  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="入库时间" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.timeruku | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="库存地点" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.productrukulocation }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="800px">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="入库时间" prop="timeruku">
          <el-date-picker v-model="temp.timeruku" type="datetime" placeholder="Please pick a date" />
        </el-form-item>
        <el-form-item label="入库地点" prop="productrukulocation">
          <el-input v-model="temp.productrukulocation" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        timeruku: [{ type: 'date', required: true, message: 'timeruku is required', trigger: 'change' }],
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.temp.timeruku = new Date(this.temp.timeruku)
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.timeruku = +new Date(tempData.timeruku) // change Thu Nov 30 2017 16:41:05 GMT+0800 (CST) to 1512031311464
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['timeruku', 'title', 'type', 'importance', 'status']
        const filterVal = ['timeruku', 'title', 'type', 'importance', 'status']
