<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="施肥时间" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.timeshifei | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="施肥次数" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.shifeinum }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="800px">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="施肥时间" prop="timeshifei">
          <el-date-picker v-model="temp.timeshifei" type="datetime" placeholder="Please pick a date" />
        </el-form-item>
        <el-form-item label="施肥次数" prop="shifeinum">
          <el-input v-model="temp.shifeinum" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        timeshifei: [{ type: 'date', required: true, message: 'timeshifei is required', trigger: 'change' }],
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.temp.timeshifei = new Date(this.temp.timeshifei)
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.timeshifei = +new Date(tempData.timeshifei) // change Thu Nov 30 2017 16:41:05 GMT+0800 (CST) to 1512031311464
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['timeshifei', 'title', 'type', 'importance', 'status']
        const filterVal = ['timeshifei', 'title', 'type', 'importance', 'status']
