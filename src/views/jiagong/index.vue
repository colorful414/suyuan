<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="包装时间" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.timebaozhuang | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="包装地点" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.productbaozhuanglocation }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="800px">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="包装时间" prop="timebaozhuang">
          <el-date-picker v-model="temp.timebaozhuang" type="datetime" placeholder="Please pick a date" />
        </el-form-item>
        <el-form-item label="包装地点" prop="productbaozhuanglocation">
          <el-input v-model="temp.productbaozhuanglocation" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        suyuanma: undefined,
        productname:'',
        productbaozhuanglocation:'',
        sort: '+id',
        timebaozhuang: new Date(),
        bozhongnum: '',
      },
      calendarTypeOptions,
      temp: {
      ...
      rules: {
        ...
        timebaozhuang: [{ type: 'date', required: true, message: 'timebaozhuang is required', trigger: 'change' }],
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.temp.timebaozhuang = new Date(this.temp.timebaozhuang)
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.timebaozhuang = +new Date(tempData.timebaozhuang) // change Thu Nov 30 2017 16:41:05 GMT+0800 (CST) to 1512031311464
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['timebaozhuang', 'title', 'type', 'importance', 'status']
        const filterVal = ['timebaozhuang', 'title', 'type', 'importance', 'status']
